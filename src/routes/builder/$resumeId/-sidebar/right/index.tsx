...
export function BuilderSidebarRight() {
  ...
  return (
    <>
      ...
      <ScrollArea
        ref={scrollAreaRef}
        className="@container bg-background sm:me-12 h-[calc(100svh-3.5rem)] overflow-hidden"
      >
...
function SidebarEdge({ scrollAreaRef }: SidebarEdgeProps) {
  ...
  return (
    <BuilderSidebarEdge side="right">
      <div className="flex-1 w-full min-h-0 overflow-x-hidden overflow-y-auto no-scrollbar">
        <div className="flex flex-col justify-center items-center gap-y-2 min-h-full">
          {rightSidebarSections.map((section) => (
            <Button
              key={section}
              size="icon"
